...
Step 1:: Switch to the tab where you have your Cloud9 environment opened.
+
Step 2:: Open the *petstore-fargate-task-definition.json* file by double clicking the filename in the left hand navigation in Cloud9.
+
...
....
+
Step 4:: We need to replace the placeholder for your account id in the template file, so that is aware of your account. The below shell script does that for you automatically using the `sed` and `aws` cli tools.
+
[source,shell]
----
...
----
+
Step 5:: Create a new task definition from the JSON file by running this command
+
[source,shell]
----
...
Step 1:: Go to the AWS Management Console, click Services then select *Elastic Container Service* under Compute.
+
...
+
Step 5:: Once your cluster is created, view your cluster and the *Services* tab should be seleced. Click *Create*.
+
...
----
+
Step 27:: Open the URL output above in your Browser.
